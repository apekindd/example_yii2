    ...
    <div id="popup2" class="popup popup_form popup_large">
        <div class="form">
            ...
            <form name="name" id="myForm" action="/esputnik/send">
                ...
                <div class="form__container">
                    <div class="form__input-container">
                        <label for="" class="form__label"><?= Yii::t('app', 'Виберіть продукт');?>
                        </label>
                        <select id="select1" class="form__select" name="<?=ES_TYPE?>" required>
                            <OPTION value="notSelected" selected>-<?= Yii::t('app', 'не вибрано');?>-</OPTION>
                            <? foreach ($arr['courses'] as $course) :?>
                                <OPTION value="<?=$this->getGroupsId("c", $course->id, "vzroslye") ?>"><?=$course->title?></OPTION>
                            <? endforeach;?>
                        </select>
                    </div>
                    <div class="form__input-container">
                    ...
                    <input type="hidden" name="<?=ES_TITLE?>" value="ЗАПИСЬ НА КУРС для взрослых">
